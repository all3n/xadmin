    ...
    <div class="head-container">
      ...
      <el-dialog :close-on-click-modal="false" :before-close="crud.cancelCU" :visible.sync="crud.status.cu > 0" :title="crud.status.title" width="500px">
        <el-form ref="form" :model="form" :rules="rules" size="small" label-width="80px">
          ...
          <el-form-item label="deploy id" prop="deployId">
            <el-input v-model="form.deployId" style="width: 370px;" />
          </el-form-item>
          <el-form-item label="deploy logs" prop="content">
      ...
      <el-table ref="table" v-loading="crud.loading" :data="crud.data" size="small" style="width: 100%;" @selection-change="crud.selectionChangeHandler" @sort-change="crud.sortChangeHandler">
        ...
        <el-table-column v-if="checkPer(['admin','xadminLog:edit','xadminLog:del'])" label="操作" width="150px" align="center">
...
<script>
import crudXadminLog from '@/api/xadminLog'
...
const defaultForm = { logId: null, deployId: null, content: null, createBy: null, updateBy: null, createTime: null, updateTime: null }
export default {
  name: 'XadminLog',
  components: { pagination, crudOperation, rrOperation, udOperation },
  mixins: [presenter(), header(), form(defaultForm), crud()],
  cruds() {
    return CRUD({ title: 'XadminLog', url: 'api/xadminLog', idField: 'logId', sort: 'logId,desc', crudMethod: { ...crudXadminLog }})
  },
  data() {
    return {
      permission: {
        add: ['admin', 'xadminLog:add'],
        edit: ['admin', 'xadminLog:edit'],
        del: ['admin', 'xadminLog:del']
      },
      rules: {
        deployId: [
          { required: true, message: 'deploy id不能为空', trigger: 'blur' }
        ],
        content: [
          { required: true, message: 'deploy logs不能为空', trigger: 'blur' }
        ]
      },
      queryTypeOptions: [
